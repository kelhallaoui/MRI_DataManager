...
def add_tumor(img, intensity, tumor_option = 'circle', diameter = 0.05, 
    ...
    Args:
        img (2d numpy array): The image
        intensity (float): The brightness of the tumor
        tumor_option (string): The kind of tumor to use
        diameter (float): A percent of the FOV
        diameter_range (list): Lower and upper bound for diameter as
                               percentage of the diameter.
        intensity_range (list): Lower and upper bound for the brightness
                                of the tumor.
    ...
    dist_x, dist_y = diameter_range[0], diameter_range[1]
        
    # The distorted circular tumor
    z = ((xx-shift_x)/dist_x)**2 + ((yy-shift_y)/dist_y)**2
    ...
    if tumor_option == 'circle':        
        ...
        z = ((xx-shift_x)/dist_x)**2 + ((yy-shift_y)/dist_y)**2
        img[z<int(rad**2)] = ndimage.filters.gaussian_filter(img[z<int(rad**2)], 
                                                             sigma = 1,
                                                             mode='constant')
        return img
